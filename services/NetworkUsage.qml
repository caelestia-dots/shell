...
Singleton {
    ...
    readonly property CircularBuffer downloadBuffer: downloadBufferImpl
    readonly property CircularBuffer uploadBuffer: uploadBufferImpl
        ...
        onTriggered: {
            ...
            if (timeDelta > 0) {
                ...
                if (root._downloadSpeed >= 0 && isFinite(root._downloadSpeed))
                    downloadBufferImpl.push(root._downloadSpeed);

                if (root._uploadSpeed >= 0 && isFinite(root._uploadSpeed))
                    uploadBufferImpl.push(root._uploadSpeed);
            }
