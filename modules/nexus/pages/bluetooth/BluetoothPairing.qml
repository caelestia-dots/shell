...
PageBase {
    ...
    ColumnLayout {
        ...
        ItemList {
            ...
            model: ScriptModel {
                values: Bluetooth.devices.values.filter(d => !d.bonded && !d.paired && !d.connected).sort((a, b) => (b.pairing - a.pairing) || a.name.localeCompare(b.name)) // qmllint disable unresolved-type
            }
