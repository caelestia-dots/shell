...
Item {
    ...
    readonly property real totalPadding:
        content.anchors.margins
        + CUtils.clamp(
            content.anchors.margins
    ...
    readonly property real nonAnimHeight:
        ((content.item as Content)?.nonAnimHeight ?? 0)
        + totalPadding
    ...
    //
    // IMPORTANT:
    // These are the original Utilities dimensions.
    // Browser does not change either one.
    //
    implicitHeight:
    ...
    implicitWidth:
        sidebar.width
        * (1 - sidebar.offsetScale)
        * horizontalStretch
        * sidebarLerp
        + Tokens.sizes.utilities.width
        * (1 - sidebarLerp)
    ...
    transitions: [
        Transition {
            ...
            Anim {
                ...
                duration:
                    Tokens.anim.durations
                        .expressiveDefaultSpatial / 2

                easing:
                    Tokens.anim.standardAccel
            }
        },
        Transition {
            to: ""
    ...
    //
    // Close browser if the phone disappears or
    // storage gets unmounted.
    //
    Connections {
        target: KdeConnect
        ...
        function onMountsChanged() {
            if (!root.browserOpen
                    || !root.browserDeviceId) {
                return;
            }

            if (!KdeConnect.isMounted(
                    root.browserDeviceId
                )) {
                root.browserOpen = false;
